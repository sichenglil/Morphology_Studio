...
./scripts/validation/test_all.ps1
```

Linux/macOS:

```bash
./scripts/maintenance/setup_dev.sh
./scripts/validation/test_all.sh
```
...
Python uses Ruff and type annotations; frontend code uses strict TypeScript, ESLint, Vitest, and Playwright. Core code must stay robot-agnostic and use `pathlib.Path`. Add dependencies only when their maintenance and license costs are justified. Record copied or modified third-party material in `project/THIRD_PARTY_NOTICES.md` and `.reuse/dep5`.
