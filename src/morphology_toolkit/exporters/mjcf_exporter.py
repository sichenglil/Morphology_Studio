...
class MjcfExporter:
    ...
    def export(self, model: RobotModel, output: Path) -> Path:
        ...
        for index, resource in enumerate(model.resources):
            if resource.kind != "mesh" or resource.uri in mesh_names:
                continue
            name = f"mesh_{index}"
        ...
        def add_body(parent, link_name, incoming=None):
            ...
            if incoming:
                ...
                if incoming.joint_type != "fixed":
                    ...
                    attrs = {
                        "name": incoming.name,
                        "type": kind,
                        "axis": " ".join(map(str, incoming.axis or (1, 0, 0))),
                    }
                    if (
                        incoming.joint_type != "continuous"
                        and "lower" in incoming.limit
                        and "upper" in incoming.limit
                    ):
                        attrs.update(
                            limited="true",
                            range=f"{incoming.limit['lower']} {incoming.limit['upper']}",
                        )
            ...
            if link.inertial:
                ...
                ET.SubElement(
                    ...
                    pos=" ".join(map(str, link.inertial.origin.xyz)),
                    fullinertia=" ".join(map(str, (ixx, iyy, izz, ixy, ixz, iyz))),
                )
            for visual in link.visuals:
                geometry = visual.geometry
                attrs = {
                    "group": "1",
                    "pos": " ".join(map(str, visual.origin.xyz)),
                    "euler": " ".join(map(str, visual.origin.rpy)),
                }
                if geometry.kind == "mesh" and geometry.resource:
                    ...
                    attrs.update(
                        type="box",
                        size=" ".join(str(value / 2) for value in geometry.size or (1, 1, 1)),
                    )
                elif geometry.kind == "sphere":
                    attrs.update(type="sphere", size=str((geometry.size or (1,))[0]))
                elif geometry.kind in {"cylinder", "capsule"}:
                    size = geometry.size or (1, 1)
                    attrs.update(type=geometry.kind, size=f"{size[0]} {size[1] / 2}")
                else:
                    continue
