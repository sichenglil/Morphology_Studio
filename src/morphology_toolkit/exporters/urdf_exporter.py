...
def _origin(parent: ET.Element, transform: Transform) -> None:
    ET.SubElement(parent, "origin", xyz=" ".join(map(str, transform.xyz)), rpy=" ".join(map(str, transform.rpy)))
...
def _geometry(parent: ET.Element, geometry: GeometryModel) -> None:
    ...
    if geometry.kind == "mesh" and geometry.resource:
        ET.SubElement(node, "mesh", filename=geometry.resource.uri.replace("\\", "/"), scale=" ".join(map(str, geometry.scale)))
