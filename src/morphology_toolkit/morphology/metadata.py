...
def generate_morphology(model: RobotModel) -> Dict[str, Any]:
    ...
    movable = [joint for joint in model.joints.values() if joint.joint_type not in {"fixed", "floating", "planar"}]
    return {
        ...
        "end_effectors": [{"link": name, "source": "kinematic_leaf", "confirmed": False} for name in leaves],
        "sensors": sorted(model.sensors),
        "actuators": sorted(model.actuators),
        "joint_limits_normalized": [{"joint": joint.name, "lower": joint.limit.get("lower"), "upper": joint.limit.get("upper")} for joint in movable],
