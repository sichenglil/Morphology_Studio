...
def _prefixed(model: RobotModel, prefix: str) -> tuple:
    ...
    result.transmissions = {
        f"{prefix}{name}": transmission for name, transmission in result.transmissions.items()
    }
...
def analyze_assembly(
    models: Dict[str, RobotModel], connections: List[AssemblyConnection], prefixes: Dict[str, str]
) -> AssemblyAnalysis:
    analysis = AssemblyAnalysis()
    if len(models) != len(set(models)):
        analysis.diagnostics.append(
            Diagnostic("ERROR", "duplicate_model", "Model IDs must be unique")
        )
    ...
    for connection in connections:
        for model_id, link, role in (
            (connection.parent_model, connection.parent_link, "parent"),
            (connection.child_model, connection.child_link, "child"),
        ):
            if model_id not in models:
                analysis.diagnostics.append(
                    Diagnostic(
                        "ERROR",
                        "unknown_model",
                        f"Unknown {role} model {model_id!r}",
                        connection.name,
                    )
                )
                ...
                analysis.diagnostics.append(
                    Diagnostic(
                        "ERROR",
                        "unknown_link",
                        f"Unknown {role} link {link!r} in {model_id!r}",
                        connection.name,
                    )
                )
        if connection.joint_type != "fixed":
            analysis.diagnostics.append(
                Diagnostic(
                    ...
                    "Version 0.1 executes fixed assembly joints only",
                    connection.name,
                )
            )
    analysis.requires_confirmation = bool(
        analysis.conflicts or any(item.severity == "ERROR" for item in analysis.diagnostics)
    )
    return analysis
...
def assemble_models(
    ...
    if any(item.severity == "ERROR" for item in analysis.diagnostics):
        raise ValueError(
            "Assembly analysis failed: " + "; ".join(item.message for item in analysis.diagnostics)
        )
