...
class PackageResolver:
    def __init__(self, explicit_map: Dict[str, Path] = None, roots: Iterable[Path] = (), mode: ProcessingMode = ProcessingMode.ASSISTED):
        self.explicit_map = {name: Path(path).resolve() for name, path in (explicit_map or {}).items()}
    ...
    def package_name(directory: Path) -> Optional[str]:
        ...
        try: return ET.parse(manifest).getroot().findtext("name")
    ...
    def discover(self, roots: Iterable[Path] = None) -> Dict[str, List[Path]]:
        ...
        for root in roots or self.roots:
            ...
            manifests = [root / "package.xml"] if (root / "package.xml").exists() else root.rglob("package.xml") if root.exists() else []
            for manifest in manifests:
                name = self.package_name(manifest.parent)
                if name: found.setdefault(name, []).append(manifest.parent.resolve())
        return found
    ...
    def resolve(self, package_name: str) -> Optional[Path]:
        ...
        if package_name in self.explicit_map:
            ...
            trace.selected = path; self._traces[package_name] = trace; return path
        discovered = self.discover()
        trace.searched.extend(path.as_posix() for path in self.roots)
        paths = list(dict.fromkeys(discovered.get(package_name, [])))
        trace.candidates.extend(ResolutionCandidate(package_name, path, "package_root", 2) for path in paths)
        if len(paths) == 1:
            trace.selected = paths[0]
        elif len(paths) > 1:
            trace.error = f"Ambiguous package {package_name!r}: {paths}"
            if self.mode == ProcessingMode.MANUAL: trace.selected = paths[0]
        else:
    ...
    def require(self, package_name: str) -> Path:
        ...
        if result is None: raise LookupError(self.explain(package_name).error)
        return result

    def explain(self, package_name: str) -> ResolutionTrace:
        if package_name not in self._traces: self.resolve(package_name)
