...
def _version(command):
    try:
        ...
        result = subprocess.run(
            [executable, *command[1:]], capture_output=True, text=True, timeout=10
        )
...
def audit_repository(root: Path) -> Dict[str, Any]:
    ...
    for candidate in (
        ...
        if candidate.exists():
            report["isaac_sim_candidates"].append(candidate.as_posix())
    destination = root / "build" / "reports"
    destination.mkdir(parents=True, exist_ok=True)
    (destination / "environment_report.json").write_text(
        json.dumps(report, indent=2), encoding="utf-8"
    )
    inventory = {"candidates": report["model_candidates"], "count": len(report["model_candidates"])}
    (destination / "model_inventory.json").write_text(
    ...
    lines = [
        ...
        f"- ROS distro: `{report['ros_distro'] or 'not active'}`",
        f"- Xacro executable: `{report['xacro'] or 'not found'}`",
        f"- Entry candidates: {len(report['model_candidates'])}",
        "",
        "## Candidate entries",
        "",
    ]
    lines.extend(
        f"- `{item['path']}` — {item['format']}, {item['confidence']} ({item['score']:.2f})"
        for item in report["model_candidates"]
    )
