...
class XacroImporter(Importer):
    def analyze(self, path: Path) -> ImportAnalysis:
        ...
        analysis.format_candidates.append(
            Candidate(path, "xacro", 1.0, "Xacro namespace or macro elements detected")
        )
        analysis.parameter_candidates = {
            name: default or None for name, default in ARG_RE.findall(text)
        }
        if (
            "<xacro:macro" in text
            and "<robot" in text
            and not re.search(r"<xacro:[\w.-]+\s", text.replace("<xacro:macro", ""))
        ):
            analysis.diagnostics.append(
                "File appears to define macros only; a wrapper entry may be required"
            )
        return analysis
    ...
    def execute(
        ...
        missing = [
            name
            for name, default in analysis.parameter_candidates.items()
            if default is None and name not in arguments
        ]
        if missing:
        ...
        if expansion.returncode:
            raise RuntimeError(
                f"Xacro failed ({expansion.returncode}) via {expansion.method}:\n{expansion.stderr.strip()}"
            )
        xml = expansion.xml
    ...
    def expand(
        self, path: Path, arguments: Dict[str, str], resolver: PackageResolver
    ) -> XacroExpansion:
        try:
            import xacro
        ...
        if xacro is not None:
            ...
            try:
                ...
                resolutions = {
                    name: trace.selected.as_posix() if trace.selected else None
                    for name, trace in resolver._traces.items()
                }
                return XacroExpansion(
                    xml, "python_api", 0, stdout.getvalue(), stderr.getvalue(), resolutions
                )
        ...
        return XacroExpansion(
            ...
            127,
            "",
            "Xacro is unavailable. Install it with 'python -m pip install xacro'.",
        )

    @staticmethod
    def expand_to(
        path: Path, output: Path, arguments: Dict[str, str], package_paths: Optional[list] = None
    ) -> Path:
