...
class DirectoryImporter(Importer):
    ...
    def analyze(self, path: Path) -> ImportAnalysis:
        ...
        for candidate_path in sorted(p for p in path.rglob("*") if p.is_file()):
            ...
            if fmt == "srdf":
                analysis.format_candidates.append(
                    Candidate(
                        ...
                        fmt,
                        1.0,
                        "Semantic Robot Description detected; excluded as a geometry entry",
                    )
                )
                continue
            ...
            analysis.entry_candidates.append(
                Candidate(candidate_path, fmt, min(score, 0.95), "; ".join(reasons), score < 0.8)
            )
    ...
    def execute(
        ...
        if selected:
            ...
            and analysis.entry_candidates
            and analysis.entry_candidates[0].score >= 0.8
            and (
                len(analysis.entry_candidates) == 1
                or analysis.entry_candidates[0].score > analysis.entry_candidates[1].score
            )
        ):
