...
def _transform(element: Optional[ET.Element]) -> Transform:
    ...
    return Transform(
        ...
        _numbers(element.get("rpy"), 3, (0.0, 0.0, 0.0)),
    )
    ...
    def analyze(self, path: Path) -> ImportAnalysis:
        ...
        try:
            ...
            for index, name in enumerate(leaves):
                ...
                analysis.interface_candidates.append(
                    {
                        "link": name,
                        "score": score,
                        "confidence": "high" if score >= 0.8 else "medium",
                        "reason": "kinematic leaf link",
                        "requires_confirmation": score < 0.8,
                    }
                )
    ...
    def execute(
        ...
        for material in root.findall("material"):
            ...
            if name:
                ...
                rgba = (
                    _numbers(
                        color.get("rgba") if color is not None else None, 4, (0.0, 0.0, 0.0, 1.0)
                    )
        ...
        for link in root.findall("link"):
                    ...
                    if geometry:
                        if tag == "visual":
                            ...
                            target.append(
                                VisualModel(
                                    geometry,
                                    _transform(node.find("origin")),
                                    material_node.get("name")
            ...
            if inertial is not None:
                ...
                if mass is not None and inertia is not None:
                    ...
                    item.inertial = InertialModel(
                        float(mass.get("value", "nan")),
                        tuple(float(inertia.get(key, "nan")) for key in keys),
                        _transform(inertial.find("origin")),
                    )
        ...
        for joint in root.findall("joint"):
            ...
            model.joints[name] = JointModel(
                ...
                axis=_numbers(axis.get("xyz") if axis is not None else None, 3, (1.0, 0.0, 0.0))
                if axis is not None
                else None,
                limit={
                    key: float(value)
        ...
        model.extension_elements = [
            ET.tostring(child, encoding="unicode")
            for child in root
            if child.tag.rsplit("}", 1)[-1] not in known
        ]
        known_uris = {resource.uri for resource in model.resources}
        for child in root:
            if child.tag.rsplit("}", 1)[-1] in known:
                continue
            for node in child.iter():
                ...
                for value in values:
                    if value.startswith("package://") and value not in known_uris:
                        model.resources.append(ResourceReference(value, kind="extension"))
                        known_uris.add(value)
        return model
    ...
    def _geometry(node: Optional[ET.Element], model: RobotModel) -> Optional[GeometryModel]:
        ...
        if kind == "mesh":
            ...
            return GeometryModel(
                kind, resource, scale=_numbers(shape.get("scale"), 3, (1.0, 1.0, 1.0))
            )
        if kind == "box":
            return GeometryModel(kind, size=_numbers(shape.get("size"), 3, (1.0, 1.0, 1.0)))
        if kind in {"sphere", "cylinder", "capsule"}:
            values = tuple(
                float(shape.get(key)) for key in ("radius", "length") if shape.get(key) is not None
            )
