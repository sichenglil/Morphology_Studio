...
def convert_mesh(
    ...
    try:
        ...
        raise RuntimeError(
            "Mesh conversion requires 'pip install morphology-toolkit[mesh]'"
        ) from exc
    ...
    if simplify_faces:
        for key, geometry in list(mesh.geometry.items()):
            if len(geometry.faces) > simplify_faces:
                mesh.geometry[key] = geometry.simplify_quadric_decimation(face_count=simplify_faces)
