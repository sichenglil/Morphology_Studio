...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build any configured Xacro model without requiring ROS"
    )
    ...
    trace = {
        ...
        "returncode": expansion.returncode,
        "stderr": expansion.stderr,
        "packages": expansion.package_resolutions,
    ...
    if forbidden:
        raise SystemExit(f"Generated XML contains unresolved tokens: {forbidden}")
    ...
    report = validate_model(model, resources)
    report.write(args.validation_json, args.validation_md)
    if args.morphology:
        args.morphology.parent.mkdir(parents=True, exist_ok=True)
        args.morphology.write_text(
            json.dumps(generate_morphology(model), indent=2), encoding="utf-8"
        )
    print(
        json.dumps(
            {
