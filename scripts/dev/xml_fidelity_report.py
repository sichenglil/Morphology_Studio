...
def counts(path: Path):
    ...
    return Counter(
        child.tag.rsplit("}", 1)[-1] for child in root if child.tag.rsplit("}", 1)[-1] not in known
    )
...
def main():
    ...
    for source in args.inputs:
        target = source.with_suffix(".roundtrip.urdf")
        UrdfExporter().export(UrdfImporter().execute(source), target)
    ...
    args.markdown.write_text(
        ...
        + "\n".join(
            f"- `{r['source']}`: preserved=`{r['preserved']}`, extensions={r['before']}"
            for r in records
        )
        + "\n",
        encoding="utf-8",
