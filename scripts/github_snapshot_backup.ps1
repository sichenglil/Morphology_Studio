...
if(-not $BackupRepository){$BackupRepository="$owner/Morphology-Conditioned-backup"}
try{Gh @('repo','view',$BackupRepository,'--json','visibility')|Out-Null}catch{Gh @('repo','create',$BackupRepository,'--private','--description','Private snapshots for Morphology-Conditioned development')|Out-Null}
