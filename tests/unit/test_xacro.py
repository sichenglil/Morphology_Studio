...
def test_python_api_xacro_with_package_find_and_space_path(tmp_path):
    ...
    package = root / "sample_description"; package.mkdir(parents=True)
    (package / "package.xml").write_text('<package><name>sample_description</name></package>', encoding="utf-8")
    (package / "part.xacro").write_text('<robot xmlns:xacro="http://www.ros.org/wiki/xacro"><xacro:macro name="part" params="prefix"><link name="${prefix}base"/></xacro:macro></robot>', encoding="utf-8")
    entry = root / "entry.xacro"
    entry.write_text('<robot xmlns:xacro="http://www.ros.org/wiki/xacro" name="sample"><xacro:arg name="prefix" default=""/><xacro:include filename="$(find sample_description)/part.xacro"/><xacro:part prefix="$(arg prefix)"/></robot>', encoding="utf-8")
...
def test_missing_package_is_structured_failure(tmp_path):
    ...
    entry.write_text('<robot xmlns:xacro="http://www.ros.org/wiki/xacro"><xacro:include filename="$(find absent)/part.xacro"/></robot>', encoding="utf-8")
