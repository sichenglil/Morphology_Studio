...
def test_two_model_fixed_assembly_and_prefix_rewrite(fixture_models):
    ...
    connection = AssemblyConnection("mount_joint", "fixed", "a", "tip", "b", "mount", Transform((0, 0, .1)))
...
def test_three_model_chain(fixture_models):
    ...
    models = {"arm": importer.execute(arm), "tool": importer.execute(tool), "camera": importer.execute(camera)}
    connections = [
