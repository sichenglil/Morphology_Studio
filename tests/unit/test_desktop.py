...
def test_transform_commit_revision_undo_and_urdf_writeback(tmp_path):
    ...
    response = client.post(
        "/api/workspaces/current/transforms/commit",
        json={
            "targetType": "link",
            "entityId": "tool",
    ...
    assert client.post("/api/workspaces/current/history/undo").json()["joints"][0]["origin"][
        "xyz"
    ] == [0.0, 0.0, 1.0]
    assert client.post("/api/workspaces/current/history/redo").json()["joints"][0]["origin"][
        "xyz"
    ] == [0.2, 0.3, 1.4]
    output = tmp_path / "written.urdf"
    assert (
        client.post("/api/export", json={"format": "urdf", "output": str(output)}).status_code
        == 200
    )
...
def test_transform_rejects_non_finite_and_saves_workspace(tmp_path):
    ...
    saved = client.post(
        ...
        json={"path": str(workspace), "settings": {"space": "local"}},
    )
...
def test_batch_joint_commit_revision_limits_and_single_history(tmp_path):
    ...
    model.write_text(
        ...
        '<limit lower="0" upper="0.5" effort="1" velocity="1"/></joint></robot>',
        encoding="utf-8",
    )
    app = create_app()
    ...
    client.post("/api/models/load", json={"path": str(model)})
    response = client.post(
        "/api/workspaces/current/joint-states/commit",
        json={
            "values": {"a": 0.4, "b": 0.2},
            "expectedRevision": 0,
        },
    ...
    assert (
        client.post("/api/workspaces/current/save", json={"path": str(workspace)}).status_code
        == 200
    )
    assert (
        client.post("/api/workspaces/open", json={"path": str(workspace)}).json()["joints"][0][
            "value"
        ]
        == 0.4
    )
    assert (
        client.post(
            "/api/workspaces/current/joint-states/commit",
            json={"values": {"a": 0.5}, "expectedRevision": 0},
        ).status_code
        == 409
    )
    assert (
        client.post(
            "/api/workspaces/current/joint-states/commit",
            json={"values": {"b": 2}, "expectedRevision": 3},
        ).status_code
        == 422
    )
