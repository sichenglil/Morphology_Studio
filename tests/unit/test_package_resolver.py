...
def test_relative_package_map(tmp_path):
    package = tmp_path / "external" / "generic_description"
    package.mkdir(parents=True)
    config = tmp_path / "package-map.yaml"
    config.write_text(
        "packages:\n  generic_description:\n    path: external/generic_description\n",
        encoding="utf-8",
    )
    mapping = load_package_map(config, tmp_path)
    assert mapping["generic_description"] == package.resolve()
