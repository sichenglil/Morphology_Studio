...
def test_srdf_is_visible_but_not_geometry_candidate(tmp_path):
    ...
    srdf.write_text(
        '<robot name="semantic"><group name="arm"><joint name="axis"/></group><disable_collisions link1="a" link2="b" reason="Adjacent"/></robot>',
        encoding="utf-8",
    )
...
def test_srdf_is_excluded_without_external_models(tmp_path):
    path = tmp_path / "generic.srdf"
    path.write_text('<robot name="generic"><group name="arm"/></robot>', encoding="utf-8")
