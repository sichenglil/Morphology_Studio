...
def test_package_survives_source_removal(tmp_path: Path):
    ...
    mesh = source / "shape.stl"; mesh.write_text("solid s\nendsolid s\n", encoding="ascii")
    urdf = source / "robot.urdf"
    urdf.write_text('<robot name="p"><link name="base"><visual><geometry><mesh filename="shape.stl"/></geometry></visual><collision><geometry><mesh filename="shape.stl"/></geometry></collision></link></robot>', encoding="utf-8")
...
def test_extension_resource_is_packaged(tmp_path: Path):
    package = tmp_path / "package"; package.mkdir(); config = package / "config.yaml"; config.write_text("value: 1", encoding="utf-8")
    urdf = tmp_path / "robot.urdf"; urdf.write_text('<robot name="p"><link name="base"/><gazebo><plugin name="p" filename="plugin"><parameters>package://demo/config.yaml</parameters></plugin></gazebo></robot>', encoding="utf-8")
    model = UrdfImporter().execute(urdf); output = tmp_path / "portable"
    PortablePackageExporter().export(model, output, ResourceResolver(tmp_path, package_map={"demo": package}))
