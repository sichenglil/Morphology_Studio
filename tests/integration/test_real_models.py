...
def external_package_map():
    config = ROOT / "configs/package_maps/local_models.yaml"
    try:
        return load_package_map(config, ROOT)
    except FileNotFoundError as exc:
        pytest.skip(f"External acceptance models are not installed: {exc}")


def expand_config(config_path: Path, output: Path):
    package_map = external_package_map()
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    result = XacroImporter().expand(
        ROOT / config["source"],
        {key: str(value) for key, value in config["arguments"].items()},
        PackageResolver(package_map),
    )
    ...
    assert (
        validate_model(model, ResourceResolver(output.parent, package_map=package_map)).errors == 0
    )
    return model
...
def test_real_models_expand_and_generic_assembly(tmp_path: Path):
    ...
    result = assemble_models(
        {"arm": arm, "hand": hand},
        [AssemblyConnection("test_mount", "fixed", "arm", "tool0", "hand", "world")],
        {"arm": "a_", "hand": "h_"},
    )
