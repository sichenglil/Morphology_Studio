...
def fixture_models(tmp_path: Path):
    arm = tmp_path / "arm.urdf"; arm.write_text(ROBOT, encoding="utf-8")
    tool = tmp_path / "tool.urdf"; tool.write_text(TOOL, encoding="utf-8")
    camera = tmp_path / "camera.urdf"; camera.write_text(TOOL.replace('name="tool"', 'name="camera"').replace('name="mount"', 'name="camera_frame"'), encoding="utf-8")
