...
test('loads the generic portable URDF into tree and viewport', async ({page}) => {
  ...
  await expect(page.getByText('1 Joints')).toBeVisible()
  await expect(page.getByRole('button',{name:'◎ arm_joint revolute',exact:true})).toBeVisible()
})
...
test('mouse selection and transform toolbar acceptance', async ({page}) => {
  ...
  await page.goto('/')
  await page.getByRole('button',{name:'导入模型',exact:true}).click()
  await page.getByPlaceholder('URDF、Xacro、MJCF 或模型目录').fill(path.resolve('../../examples/simple_two_link/robot.urdf'))
  await page.getByRole('button',{name:'导入并显示',exact:true}).click()
  ...
  await expect(page.getByText('已选: base')).toBeVisible()
  ...
  await expect(page.getByText('已选: arm')).toBeVisible()
  await page.screenshot({path:path.resolve('../../build/ui/acceptance/transform/kinematic_gizmo.png')})
  expect(pageErrors).toEqual([])
})

for (const model of [
  {name:'generic-two-link',path:'../../examples/simple_two_link/robot.urdf'},
