...
test('editable joint values synchronize preview, units, limits and poses',async({page})=>{
  ...
  await page.locator('.el-dialog input').first().fill(path.resolve('../../examples/simple_two_link/robot.urdf'));await page.locator('.el-dialog__footer .el-button--primary').click()
  const row=page.locator('[data-joint-id="arm_joint"]'),input=row.locator('input');await expect(input).toBeVisible();await page.screenshot({path:path.join(output,'joint_value_idle.png')})
