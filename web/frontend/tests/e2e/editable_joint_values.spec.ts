...
test('editable joint values synchronize preview, units, limits and poses',async({page})=>{
  ...
  await page.locator('.el-dialog input').first().fill(path.resolve('../../build/packages/ur5e/robot.urdf'));await page.locator('.el-dialog__footer .el-button--primary').click()
  const row=page.locator('[data-joint-id="shoulder_pan_joint"]'),input=row.locator('input');await expect(input).toBeVisible();await page.screenshot({path:path.join(output,'joint_value_idle.png')})
  ...
  await page.getByRole('button',{name:/保存姿态|淇濆瓨濮/}).click();await page.locator('.el-message-box input').fill('inspection_pose');await page.locator('.el-message-box__btns .el-button--primary').click();await page.getByRole('button',{name:/全部归零|鍏ㄩ儴褰/}).click();await page.waitForTimeout(150);await page.getByRole('button',{name:/加载|鍔犺浇/}).click();await page.waitForTimeout(150);await page.screenshot({path:path.join(output,'pose_loaded.png')})
